...
Task ID: 19
Agent: Z.ai Code (principal)
Task: Audit offensif complet du système d'abonnement KinShop (FREE vs Premium) — backend = source de vérité ?

Work Log:
- AUDIT STATIQUE (code commit 477029d = prod) : plans.ts (isPremiumActive/premiumUntil date-checked à chaque lecture), config-registry (54 paramètres, quotas plan.free.*/plan.premium.* lus FRAÎS à chaque requête, sans cache), auth.ts (sessions opaques SHA-256 en DB, rôle lu en DB, requireStoreOwner anti-IDOR), admin.ts (PIN x-admin-pin, 16 routes admin toutes gardées), grep exhaustif des écritures isPremium/premiumUntil (4 points d'écriture seulement : webhook HMAC, simulate-confirm conditionnel, admin PIN, verifyAndApplyPremium).
- AUDIT DYNAMIQUE (PROD kinshop.aenews.digital) : 4 profils de test créés via API (USER_FREE, USER_PREMIUM_ACTIVE grant 30j, USER_PREMIUM_EXPIRED grant 1j puis expiration forcée en DB, USER_NO_SUBSCRIPTION) + ADMIN (PIN). 4 scripts Python (audit-tmp/), 60+ tests curl/requests dont race conditions 12 threads, injection payload, IDOR, webhooks falsifiés, brute-force PIN, suspension, expiration réelle via SSH (UPDATE premiumUntil epoch ms — erreur initiale format ISO corrigée).
- FAILLES CONFIRMÉES PAR TESTS : F-01 CRITICAL POST /api/premium/simulate-confirm → 200 Premium gratuit +30j cumulables (cause : payments.chariowProductId vide en prod → isChariowLiveAsync=false → simulateur démo actif ; CHARIOW_API_KEY bien présente dans .env mais produit non configuré) ; F-02 CRITICAL POST /api/payments/simulate-confirm SANS auth → CMD passée "paid" par un anonyme (CMD-2026-000006 testée) ; F-03 HIGH PATCH /api/boost confirm → campagne active sans paiement + boutique FREE affichée en Sponsorisé accueil (impact public prouvé) ; F-04 HIGH boutique suspendue : GET /api/stores 200 + produits exposés, owner peut encore créer (seul orders bloque 403) ; F-05 MEDIUM 8 tentatives PIN admin sans aucune 429 (pas de rate limit) ; F-06 LOW stats agrégats totaux non clampés pour FREE (série clampée ok) ; F-07 LOW race TOCTOU théorique (12 threads → 0 dépassement, non exploité en pratique).
- PREUVES POSITIVES : quotas produits(20)/photos(1)/catégories(10)/coupons(3)/zones(5)/factures(15) tous 402 serveur ; stats clamp 7j ; domaine 402 ; anti-IDOR 7/7 = 403 ; injection PATCH stores ignorée (whitelist) ; register role=admin ignoré ; headers x-premium ignorés ; webhook Pulse 401 sans/mauvaise signature ; expiration → même session retombe plan=free immédiatement (X1) ; données pré-existantes conservées (21 produits) + nouvelles créations bloquées (règle métier exacte demandée) ; UI trompée (mock auth/me premium+admin) → API renvoie quand même 402 domaine.
- FRONTEND (agent-browser sur prod) : dashboard FREE correct (upsell, paywall domaine, stats), UI accepte un état mocké (preuve que l'UI n'est pas la sécurité) mais les actions réelles restent gardées serveur.
- CLEANUP : 3 boutiques DELETE admin (cascade), comptes/sessions/résidus supprimés en DB (0 orphelin), accueil public sans trace de test, campagne boost de test clôturée.

Stage Summary:
- RAPPORT LIVRÉ (dans le chat) : 7 fonctionnalités payantes quotas + 3 payantes non-quotas identifiées ; matrice des droits réelle code+tests ; 3 CRITICAL/HIGH liés au même pattern (simulateurs de paiement auto-activés quand la config réel est incomplète — bascule démo/prod implicite à rendre explicite) ; 2 HIGH/MEDIUM (suspension frontend-only, PIN sans rate limit) ; backend QUASI-INTÉGRALEMENT source de vérité SAUF ces brèches volontaires de démo.
- CORRECTIONS PROPOSÉES mais NON APPLIQUÉES (attente validation utilisateur) : kill-switch prod pour simulateurs, config produit Chariow (remettrait le paiement réel ET fermerait F-01), auth/signature pour simulate-confirm, garde status=suspended dans stores GET + écritures owner, rate limit PIN admin, clamp agrégats stats, transactions quotas.
- Aucun code modifié. Scripts d'audit dans /home/z/audit-tmp/ (hors dépôt Git).

---
Task ID: 20
Agent: Z.ai Code (principal)
Task: Appliquer les corrections P0 issues de l'audit d'abonnement (Task 19) — kill-switch simulateurs, Boost par autorité, suspension serveur, anti brute-force PIN — puis déployer et vérifier en production.

Work Log:
- CORRECTIF F-01/F-02 (CRITICAL) — nouveau src/lib/simulation.ts : isPaymentSimulationEnabled() lit PAYMENT_SIMULATION (off par défaut, jamais défini en prod). POST /api/premium/simulate-confirm et /api/payments/simulate-confirm : kill-switch EN TÊTE (403 avant tout traitement/fuite d'info). payments/simulate-confirm : même en démo, auth OBLIGATOIRE — acheteur de la commande (session) ou admin ; commandes legacy sans compte = admin seul ; anonyme 401, tiers 403. premium/checkout : branche sim conditionnée (503 message honnête si Chariow non configuré) ; payments/initiate : 503 si ni MOMO ni simulation.
- CORRECTIF F-03 (HIGH) — PATCH /api/boost : self-confirmation owner supprimée hors démo explicite (403 « paiement vérifié par l'administration ») ; l'activation prod passe par PATCH /api/admin/boost action=activate (existant) ou futur webhook. UI BoostPanel : reload de la liste si activation refusée (campagne visible en attente).
- CORRECTIF F-04 (HIGH) — requireStoreOwner (src/lib/auth.ts) : boutique status=suspended → 403 explicite sur TOUTE opération owner (opt-in allowSuspended pour futur documenté). GET /api/stores public : boutique suspendue → squelette minimal (slug/name/logoEmoji/status, products:[]) — l'UI vitrine affiche l'avis ⛔ existant, aucune donnée interne ni produit exposé. Accueil: les suspendues étaient déjà exclues.
- CORRECTIF F-05 (MEDIUM) — src/lib/admin.ts : notePinFailure() (rateLimit admin-pin:<ip>, 5 échecs/15 min) appliqué dans guardAdmin (TOUTES les routes admin) + POST /api/admin/auth ; succès jamais comptabilisés ; 429 « Trop de tentatives. Réessaie dans 15 minutes. »
- TESTS LOCAUX 17/17 : kill-switch 403 sur les 3 simulateurs (même avec session propriétaire) ; checkout 503 ; suspension : squelette public + écritures 403 + réactivation restaurée ; brute-force PIN 401×5→429 (route auth ET toute route admin via x-admin-pin) ; PIN correct toujours 200 ; en démo (PAYMENT_SIMULATION=on local) : initiate mode=simulation, acheteur confirme 200 paid, premium simulate owner 200, boost PATCH owner 200.
- DÉPLOIEMENT : commit f6d3520 (correctifs) + ea30b7e (update.sh root-aware) → GitHub main → VPS.
- INCIDENT DÉPLOIEMENT MAÎTRISÉ : update.sh lancé via sudo (root) → ① pm2 reload exécuté sur le démon PM2 de root (kinshop introuvable, app restée sur l'ancien build) ; ② après reload manuel aenews, le build root avait copié .next/standalone/.env en root:0600 → process aenews illisible → Prisma sans DATABASE_URL → API en 500. Réparation : chown aenews + restart (prod restaurée), puis correctif durable deploy/update.sh : re-exec automatique `runuser -u aenews` si lancé en root + chown du .env standalone après build + récupération ownership .next/node_modules. Passage complet propre re-validé (build + reload + 200).
- VÉRIFICATION PROD (kinshop.aenews.digital, build ea30b7e) : premium/simulate-confirm 403 (avant: 200) ; payments/simulate-confirm 403 (avant: 200) ; PATCH /api/boost 403 ; GET /api/home 200 ; vitrine réelle intacte ; brute-force PIN via nginx : 429 confirmés dès réutilisation d'IP (l'IP de sortie du sandbox de test tourne — limiteur par-IP fonctionnel, 5 essais/IP/15 min) ; suspension testée de bout en bout en prod (compte+boutique+produit → suspend → squelette public, écriture 403, disparition accueil → activate → DELETE via query param id) ; cleanup intégral (boutique supprimée, compte+sessions effacés en DB, 0 résidu).
- NAVIGATEUR (agent-browser sur prod) : accueil rendu (header, hero, moyens de paiement, sections), vitrine AENEWS rendue (recherche, catégories, avis), 0 erreur page/console. Helper SSH éphémère supprimé, aucun secret commité.

Stage Summary:
- Les 5 failles P0 de l'audit (F-01/F-02 CRITICAL, F-03/F-04 HIGH, F-05 MEDIUM) sont CORRIGÉES, DÉPLOYÉES et VÉRIFIÉES en production : un utilisateur sans abonnement ne peut plus obtenir Premium, « payer » une commande ou activer un Boost par requête client ; les boutiques suspendues ne servent plus aucun contenu ; le PIN admin est protégé contre le brute-force.
- La bascule démo/prod est désormais EXPLICITE : simulateur ⇔ PAYMENT_SIMULATION=on (absent en prod) ET fournisseur réel non configuré. Pour réactiver le paiement Premium réel : créer le produit Chariow (type Course/Downloadable, 3 $) et coller son ID dans Console ADMIN → Paiements (paiement live auto-alors).
- Reste P1 (proposé, non appliqué) : clamp des agrégats stats FREE (F-06), transactions quotas anti-TOCTOU (F-07).
- Production : kinshop.aenews.digital = GitHub main (ea30b7e), PM2 online, 0 résidu de test.

---
Task ID: 21
Agent: Z.ai Code (principal)
Task: Appliquer les corrections P1 issues de l'audit d'abonnement (Task 19) — F-06 clamp des agrégats stats à l'horizon du plan, F-07 quotas atomiques anti-TOCTOU — puis déployer et vérifier en production.

Work Log:
- CORRECTIF F-06 (LOW) — GET /api/stats : les agrégats « totaux » n'étaient pas clampés (série seule clampée). Nouveau planSince = début de la fenêtre autorisée par statsDays (≤ since) ; db.storeVisit.aggregate (views.total) et allOrders (orders.total, repeatCustomers) filtrés sur createdAt/day ≥ planSince. Un compte FREE (7 j) ne peut plus lire de compteurs all-time ; un Premium voit l'horizon 60 j même s'il demande days=14 (planSince ≤ since : la fenêtre demandée reste toujours couverte).
- CORRECTIF F-07 (LOW) — nouveau src/lib/quota-guard.ts : withQuotaLock (mutex asynchrone en-processus par clé) + withStoreQuotaWrite (mutex par boutique + transaction Prisma interactive). Le count et le create partagent DÉSORMAIS la même unité atomique — impossible de dépasser un quota par course critique. Appliqué aux 5 sites à quota : POST products, coupons, delivery-zones, store-categories (dédup + quota + create dans la même tx, ce qui ferme aussi la course sur les doublons), invoices (quota mensuel). Les pré-checks existants sont conservés en amont pour l'UX (même précédence d'erreurs), la vérification FAIS FOI est celle de la transaction. SQLite : BEGIN IMMEDIATE (vu en dev.log) → sérialisation effective ; échec = fail-closed.
- TESTS LOCAUX 24/24 (/home/z/p1-tmp, hors dépôt, supprimé) : course 25 POST produits parallèles → exactement 19×201 + 6×402, 20 en base (jamais > quota) ; course 8 coupons → 3 en base exactement ; doublons 409 sous le quota ; zones 5×201 puis 402 ; catégories 201 + doublon 409 ; facture 201 ; F-06 avec données rétrodatées (2 commandes + 50 vues à J-12) : FREE views.total=7 (pas 57), orders.total=1 (pas 3), repeatCustomers=0, série 7 pts ; bascule Premium en DB → views.total=57, orders.total=3, repeatCustomers=1, série 14 pts (symétrie parfaite). 2 faux échecs initiaux identifiés comme artefacts du script (codes HTTP concaténés sans newline + tests de doublon placés après remplissage du quota — le 402 préempte à juste titre le 409) ; suite corrigée → 24/24.
- DÉPLOIEMENT : commit 1bab803 → GitHub main → update.sh en tant qu'aenews (SFTP + script distant pour éviter les conflits de quoting bash) ; build + pm2 reload OK, PM2 online (fork mono-processus — le mutex en-processus est valide pour la topologie actuelle), home 200. Bun absent du PATH non-login : purge exécutée via $HOME/.bun/bin/bun avec script temporaire DANS /opt/KINSHOP (résolution Prisma du projet ; hors projet → « Cannot find module .prisma/client »).
- VÉRIFICATION PROD (127.0.0.1:3310) : F-07 course 10 coupons parallèles → 3×201 + 7×402, exactement 3 en base ; F-06 stats FREE days=30 → serie:7, plan.statsDays:7 ; régression P0 : premium/simulate-confirm et payments/simulate-confirm toujours 403. Cleanup intégral : boutique de test supprimée (le DELETE /api/stores en owner n'aboutit pas — réservé admin —, suppression directe en DB avec cascade), compte + sessions purgés → 0 store, 0 user résiduel.
- NAVIGATEUR (agent-browser sur dev local) : accueil rendu (header, hero, moyens de paiement, sections), 0 erreur page/console. Helpers SSH éphémères supprimés (aucun secret résiduel), tmp-p1 retiré du dépôt.

Stage Summary:
- Les 7 failles de l'audit d'abonnement sont désormais TOUTES corrigées, déployées et vérifiées (P0 : F-01→F-05 en Task 20 ; P1 : F-06/F-07 en Task 21). Les quotas du plan sont incompressibles même sous parallélisme, et les statistiques respectent l'horizon du plan sur TOUS les agrégats.
- Note produit (implication F-06) : « au total » dans le dashboard signifie désormais « sur l'horizon du plan » (7 j FREE / 60 j Premium) — plus de compteur all-time, c'est le principe même du paywall d'historique.
- Production : kinshop.aenews.digital = GitHub main (1bab803), PM2 online, 0 résidu de test.

---
Task ID: 22
Agent: Z.ai Code (principal)
Task: P2 (plan marketplace) — Catégories : complétion de bout en bout (vitrine, dashboard, accueil marketplace), tests, déploiement, vérification production complète (exigence utilisateur : tests prod systématiques en fin de phase).

Work Log:
- AUDIT PRÉALABLE : le volet catégories V10 avait un backend solide (GlobalCategory admin CRUD + StoreCategory owner CRUD avec quota/dédup/rate-limit + Product.storeCategoryId validé + GET /api/categories public) mais 4 brèches UI/API le rendaient INVISIBLE pour les clients : ① GET /api/stores?slug= n'exposait pas les storeCategories ; ② la vitrine filtrait sur l'ancien texte libre Product.category (les rayons du owner ne servaient à rien côté client) ; ③ le formulaire produit du dashboard n'envoyait jamais storeCategoryId (0 usage) ; ④ l'accueil marketplace n'exploitait pas les catégories globales (aucune navigation).
- API : GET /api/stores?slug= inclut désormais storeCategories (actives, ordonnées, icône+nom de la globale rattachée, forme StoreCategoryData aplatie) ; GET /api/home accepte ?cat=<slug global> — résout la catégorie (404 si inconnue/inactive), filtre sponsorisés/populaires/nouveautés (boutiques ayant ≥1 produit dans la catégorie, via StoreCategory.active+globalCategoryId+products.some) ET les produits récents (storeCategoryId ∈ rayons de la catégorie), renvoie categories (12 actives) + category (meta active).
- VITRINE (store-view.tsx) : puces de rayons structurées « ✨ Tout (n) » + « {icône globale} Nom (effectif) » avec compteur par rayon ; filtre sur p.storeCategoryId prioritaire sur le legacy ; fallback automatique sur les puces texte libre si la boutique n'a AUCUN rayon (rétrocompat totale) ; filtre auto-réparant (rayon supprimé en session → retombe sur « Tout »).
- DASHBOARD : formulaire produit (création + édition) avec sélecteur « Rayon de la vitrine » (optionnel, sentinel « — Aucun rayon — » car Radix refuse value="") ; payload POST conditionnel + PATCH null = détachement (serveur valide l'appartenance) ; cartes produit affichent le nom du rayon sinon legacy.
- ACCUEIL (marketplace-home.tsx) : barre « Explorer par catégorie » (icônes, chips, aria-pressed, refetch /api/home?cat=, état « Filtrage… », légende catégorie active) ; affichée seulement si ≥1 catégorie globale active.
- TESTS LOCAUX 26/26 (/home/z/p2-tmp, supprimé) : rayons + doublon 409 + GET owner ; produits rattachés (201, storeCategoryId persisté, id invalide 400, produit sans rayon) ; PATCH attach/detach (200 + vérifié en lecture) ; vitrine publique (2 rayons, icône globale 🧪, produits portent storeCategoryId) ; désactivation de rayon → disparaît de la vitrine, produits conservés sous « Tout » ; /api/home (categories ≥1, ?cat= → 2 produits filtrés + meta + boutique incluse, ?cat=inconnu → 404). Navigateur local : puces « ✨ Tout (3) / 🧪 Électronique (2) / 🏷️ Mode (1) », filtrage au clic vérifié, barre accueil visible, 0 erreur console.
- DÉPLOIEMENT : commit 2fa8a51 → GitHub main → update.sh en tant qu'aenews → PM2 online, home 200.
- VÉRIFICATION PROD COMPLÈTE (exigence systématique) : API — 12 catégories globales actives, rayons de test créés et exposés avec icône 📱, 3 produits rattachés, detach vérifié en relecture live, /api/home categories:12 + ?cat=telephones → produits A1/A2 filtrés + boutique dans newest, ?cat=inconnu → 404 ; RÉGRESSIONS — premium/payments simulate-confirm 403, race 10 coupons → 3×201+7×402 (exactement 3 en base), /api/stats sans session → 401 (garde intacte ; F-06 entièrement revalidé en prod à la Task 21, route non modifiée par P2 — un check annexe a échoué uniquement par faute de syntaxe jq du script de test) ; NAVIGATEUR — accueil prod : barre « Explorer par catégorie » avec les 12 catégories réelles, « 📱 Téléphones » actif (capture d'écran), vitrine réelle aenews-store : rayon « Abonnement NETFLIX (0) » avec produit legacy non rattaché visible sous « Tout » (transition legacy exacte), état vide gracieux « Aucun produit trouvé » au clic sur le rayon vide, 0 erreur page/console ; CLEANUP — boutique + compte de test purgés (0 store, 0 user p2prod), helpers SSH éphémères supprimés, 0 secret commité.

Stage Summary:
- P2 CATÉGORIES livré en production (2fa8a51) : les rayons gérés par les propriétaires sont désormais VISIBLES et UTILISABLES par leurs clients (navigation vitrine par rayons avec compteurs et icônes), les produits se rattachent aux rayons depuis le dashboard, et l'accueil marketplace offre une navigation par catégories globales avec filtrage honnête (boutiques ET produits).
- Rétrocompatibilité garantie : boutiques sans rayons → ancien comportement (puces texte libre) ; produits non rattachés → visibles sous « Tout » ; transition legacy vérifiée sur la vraie vitrine aenews-store.
- Méthode désormais standardisée (demande utilisateur) : chaque phase se termine par une batterie de tests en production (API + navigateur + régressions P0/P1) avant délivrance.
- Production : kinshop.aenews.digital = GitHub main (2fa8a51), PM2 online, 0 résidu de test, 0 erreur console.

---
Task ID: 23
Agent: Z.ai Code (principal)
Task: P4 (plan marketplace) — Factures intégrité : empreinte V2 contenu complet, immuabilité documentaire, QR de vérification public, synchronisation commande↔facture — implémentation, tests locaux, déploiement, vérification production complète + nettoyage (exigence utilisateur : tests prod systématiques).

Work Log:
- AUDIT PRÉALABLE (8 brèches) : F4-1 factures libres KinFacture sans hash ni numéro séquentiel (verify renvoyait hashMatch=true pour tout hash vide) ; F4-2 hash v1 ne couvrant ni les lignes ni le client (altération de contenu indétectable) ; F4-3 DELETE libre d'une facture envoyée/payée (justificatif client détruit) ; F4-4 absence de graphe de statuts (boucles paid→sent→draft, annulation impossible, zéro audit) ; F4-5 QR de vérification jamais imprimé ni lié (/api/invoices/verify inutilisé, pas de page) ; F4-6 commande annulée/remboursée laissant sa facture active ; F4-7 race TOCTOU sur « une seule facture active par commande » (findFirst+create hors tx) ; F4-8 facture de commande invisible côté vendeur ET client.
- LIBS : invoice-integrity.ts — invoiceHashV2 (empreinte sur TOUT le contenu : totaux + lignes canoniques + client + téléphone + échéance + note, sel v2), versioning rétrocompatible (expectedInvoiceHash recalcule v2 si version≥2, v1 historique sinon, "" si héritée), makeSequentialKinfactureNumber (Counter atomique KF-YYYY-NNNNNN, remplace l'ancien numéro aléatoire — les anciens numéros restent résolubles). kinfacture.ts : InvoiceStatus étendu (cancelled/credited), buildInvoiceVerifyUrl. auth.ts : helper conflict(409). order-workflow.ts : type OrderEventType + invoice_cancelled.
- API : POST /api/invoices → numéro séquentiel + hash V2 + version=2 DANS la transaction quota (mutex boutique — pas de trou de séquence) + logAudit ; PATCH → graphe serveur draft→sent→paid, cancelled depuis tout état actif (motif ≥4 chars, note préfixée [ANNULÉE — motif], refusé sur facture rattachée à une commande), cancelled/credited terminaux, audit à chaque transition ; DELETE → brouillons uniquement (409 sinon) ; /api/orders/invoice → contrôle « une seule active » + création + OrderEvent dans UNE transaction atomique (F4-7) + hash V2 ; /api/orders PATCH → commande cancelled/returned/refunded ⇒ annulation AUTOMATIQUE des factures actives (note [ANNULÉE AUTO], OrderEvent invoice_cancelled, audit) ; /api/invoices/verify → verdict selon version (V2 intégral / V1 historique / legacy sans empreinte : valid=true+legacy=true, message honnête), distinguo cancelled, aucun PII exposé, fix : legacy prime sur hashMatch dans valid.
- UI : canvas facture — QR AUTHENTICITÉ imprimé (→ /#/verifier/{number}) + empreinte 8 hex sous le numéro + statuts ANNULÉE/AVOIR ; nouvelle page publique #/verifier/{number} (verdict Authentique/Annulée/Non authentique/Inconnue, explications, aucune donnée perso) ; bandeau d'authenticité sur #/facture/ (ok/legacy/cancelled/bad) ; dashboard vendeur — bouton Annuler avec dialog motif obligatoire (suppression réservée aux brouillons), badges annulée/avoir ; bouton « Émettre la facture » + lien facture dans OrderWorkflowControls ; lien facture cliquable dans my-orders (client) ; routing #/verifier complet (parseHash, hydratation, popstate, hash-sync, case render).
- INCIDENTS CORRIGÉS : ① ownership .git du VPS contaminé par root (résidu Task 20) bloquant git fetch → chown -R aenews via sudo puis redéploiement propre ; ② deep-link #/verifier tombait sur la landing — un MultiEdit atomique antérieur avait laissé le case render + branche popstate non appliqués → corrigés et redéployés ; ③ bandeau facture publique affichait « contenu intégral vérifié » pour une facture héritée → le cas legacy prime sur valid ; ④ bandeau « facture inconnue » citait le champ de saisie → checkedNumber (numéro réellement vérifié).
- TESTS LOCAUX 57/57 (/home/z/p4-tmp, supprimé en fin) : numérotation séquentielle continue (KF-…000003→000004), hash 64 hex + version 2, falsification DB directe DÉTECTÉE puis revalidation après restauration, graphe complet (draft→sent→paid, DELETE 409 sur émises, transitions interdites 400, annulation motif court 400/long 200 + note préfixée, cancelled terminal), brouillon supprimable, facture de commande (INV-, source=order, verify valide, accès client 200 / anonyme 401), race 6 POST parallèles → exactement 1×201 + 5×409, annulation de commande → facture auto-annulée + verify cancelled, refacturation impossible 400, quota 15/mois → 402 (15 en base), régressions (simulateurs auth-gardés en démo locale, stats 401, home 200). Rétrocompat prouvée par factures héritées recréées : sans empreinte → legacy=true valid=true ; hash V1 pré-P4 (formule historique) → hashMatch=true.
- DÉPLOIEMENT : commits 54734c6 (P4) → 782036e (fix verify legacy) → dcc8b24 (fix routing #/verifier) → b708c3f (checkedNumber) → GitHub main → update.sh en tant qu'aenews (réparation sudo chown .git préalable) — build + pm2 reload OK, home 200 à chaque étape.
- VÉRIFICATION PROD (127.0.0.1:3310 + domaine public) : 50/51 API — séquence/hash/version, falsification détectée, graphe intégral, race 1×201+5×409, annulation auto, quotas, simulateurs 403 (kill-switch), stats 401, home 200, 12 catégories (P2) ; facture héritée simulée en prod → legacy=true valid=true ; UNIQUE échec = KF-DEMO01 absente en prod (seed jamais exécutée en prod — non pertinent). NAVIGATEUR PROD : #/verifier/INV-2026-000002 → « Facture authentique » + émetteur/montant/date/origine + mention empreinte cryptographique ; #/verifier/KF-2026-000001 → « Facture annulée » ; #/verifier/inconnu → « Facture inconnue de la plateforme » ; #/facture/INV-2026-000002 → bandeau « Facture authentique — contenu intégral vérifié par KinShop » + lien Vérifier, canvas avec Empreinte 29DD912D imprimée + QR authenticité confirmé (9386 px sombres à la position exacte) ; LOCAL navigateur : dashboard vendeur → onglet Commandes → bouton « Émettre la facture » → toast + lien → page publique ; dialog d'annulation (motif obligatoire, bouton désactivé <4 chars, badge Annulée) ; 0 erreur console.
- CLEANUP : boutique de test prod p4-prod-check supprimée (cascade), 2 comptes p4prod* + sessions purgés, facture héritée simulée effacée → 0 store/0 user/0 facture de test ; tmp-p4p + scripts de test retirés du VPS ; DB locale purgée (factures legacy + scénario UI, 0 résidu) ; helpers SSH éphémères supprimés, 0 secret commité (PAT GitHub préexistant toujours à révoquer).

Stage Summary:
- P4 FACTURES INTÉGRITÉ livré en production (b708c3f) : chaque facture KinShop porte désormais une empreinte sha256 de son contenu INTÉGRAL (v2) et un numéro séquentiel sans trou ; une facture émise ne peut plus être supprimée ni modifiée silencieusement (rectification = annulation tracée avec motif) ; le client peut VÉRIFIER l'authenticité de son document (QR imprimé → page publique → verdict signé plateforme) ; les commandes annulées/retournées/remboursées annulent automatiquement leurs factures ; les factures antérieures restent valides (rétrocompat v1 et héritées, testé en prod).
- La facturation KinShop respecte les principes comptables de base : séquence continue, immuabilité, pièce annulée conservée, traçabilité complète (journal d'audit + OrderEvents).
- Méthode standardisée maintenue : tests API locaux (57/57) → déploiement → tests prod (50/51 API + navigateur) → régressions P0/P1/P2 (simulateurs 403, stats 401, quotas, catégories) → purge intégrale (0 résidu) → 0 erreur console.
- Production : kinshop.aenews.digital = GitHub main (b708c3f), PM2 online, 0 résidu de test, 0 erreur.
- Reste P3 (paiements — abstraction provider, dépend de la config Chariow : clé API présente, produit à créer), P5 (gouvernance : reports/verification/users/audit), P6 (boost/home), P7 (analytics uniques) selon le plan marketplace.

---
Task ID: 24
Agent: Z.ai Code (principal)
Task: P5 (plan marketplace) — Gouvernance : suspension de comptes, chaîne d'intégrité du journal d'audit, vérification des boutiques de bout en bout, durcissement des signalements — implémentation, tests locaux, déploiement, vérification production complète + nettoyage (exigence utilisateur : tests prod systématiques).

Work Log:
- AUDIT PRÉALABLE (8 brèches) : F5-1 badge « Vérifiée » invisible (verificationStatus jamais exposé dans /api/home, aucun badge UI vitrine/accueil, action verify admin SANS aucune UI — fonctionnalité morte de bout en bout) ; F5-2 aucun workflow de demande de vérification par le propriétaire ; F5-3 pas de suspension de COMPTE utilisateur (spammeur/fraudeur inbloquable, sessions non révocables) ; F5-4 journal d'audit falsifiable en DB sans détection (P4 avait blindé les factures, pas le journal) ; F5-5 zéro trace des événements d'authentification (login/échec/register/logout) ; F5-6 signalements incomplets (pas de suivi rapporteur, clôture sans note possible, commande d'autrui signalable, aucune passerelle vers la modération) ; F5-7 admin/users sans garde « dernier admin » ni auto-protection ni statut ; F5-8 graphe de transitions des signalements non appliqué côté serveur.
- SCHÉMA (migration additive) : User.status/suspendedAt/suspendedReason ; Store.verificationRequestedAt ; AdminAction.prevHash/hash. db push OK local + prod.
- LIB : audit.ts — CHAÎNE D'INTÉGRITÉ du journal : hash = sha256(prevHash | createdAt | payload) sur le contenu réellement stocké (slicing avant hachage, createdAt figé), écritures sérialisées par mutex en-processus (withQuotaLock "audit:chain" — topologie PM2 fork mono-processus), verifyAuditChain() (recalcul intégral + contrôle d'enchaînement, entrées héritées pré-P5 comptabilisées mais non vérifiées — rétrocompat). logAdminAction délègue à logAudit (TOUTES les actions admin entrent dans la chaîne). auth.ts : getUserFromRequest fail-closed (compte suspendu → null).
- AUTH (F5-3/F5-5) : login refusé 403 pour compte suspendu + auth.login_blocked journalisé ; auth.login / auth.login_failed / auth.register / auth.logout journalisés avec IP (jamais de mot de passe). Suspension admin = transaction [user.update suspended + session.deleteMany] → déconnexion immédiate partout ; getUserFromRequest couvre toute session résiduelle (fail-closed). GET /api/auth/me renvoie user:null (comportement visiteur) — cohérent.
- ROUTES : POST /api/stores/verify-request (F5-2 : owner anti-IDOR, boutique active, 1 demande en cours max, cooldown governance.verificationCooldownDays=7 j paramétrable console, rate limit 3/24 h/compte, logAudit owner) ; GET /api/reports/mine (F5-6 : suivi par le rapporteur) ; PATCH /api/admin/reports — graphe serveur (open→under_review→action_required→resolved/dismissed, terminaux), note OBLIGATOIRE (≥4 chars) pour clôturer, passerelle action=suspend_store (suspend la boutique ciblée + report→action_required, transaction) ; PATCH /api/admin/users — role + suspend (motif ≥4 chars OBLIGATOIRE, jamais un admin, jamais double) / unsuspend, garde anti-lockout dernier admin actif, recherche serveur q + stats.suspended ; GET /api/admin/logs?verify=1 → verdict {intact, checked, legacy, brokenAt} ; /api/home expose verified (bool) sur sponsored/popular/newest.
- UI : vitrine — badge « Vérifiée » (ShieldCheck) à côté du nom ; accueil — icône vérifiée sur cartes sponsorisées/populaires ; dashboard vendeur — bandeau vérification 4 états (unverified/pending/verified/rejected) + bouton « Demander la vérification » + badge header ; console admin — sous-menu « Vérification » par boutique (vérifiée/en attente/rejetée/réinitialiser), onglet Utilisateurs (badge Suspendu + boutons Suspendre/Réactiver + compteur), onglet Signalements (clôture avec note obligatoire via prompt, bouton « Suspendre cette boutique », décision affichée), onglet Journal (bandeau explicatif + bouton « Vérifier l'intégrité » + verdict coloré intact/compromis, filtres entityType auth/user). Config : 1 spec governance.verificationCooldownDays + section Gouvernance (icône ShieldCheck).
- FIX LATENT au passage : premium.ts — updateMany filtrait chariowSaleId:null sur un champ String @default("") NON nullable (rejet Prisma au runtime, activation Premium réelle jamais testée avec vraie vente) → filtre sur chaîne vide ; TS 0 erreur.
- TESTS LOCAUX 62/62 (/home/z/p5-tmp, supprimé) : demande vérification (200/double 409/401/403 autrui/pending public) ; badge exposé (verified public + home bool) ; auth journalisé (register/login_failed/login/logout) ; chaîne — intacte, ≥5 scellées, FALSIFICATION DB DÉTECTÉE (detail modifié → brokenAt raison explicite) puis intacte après restauration fidèle, 401 sans PIN ; signalements — commande d'autrui 403, par son client 201, doublon 409, reports/mine, clôture sans note 400, open→resolved 400, graphe OK, resolved terminal, suspend_store 200 → boutique réellement suspendue ; suspension — motif obligatoire 400, sessions révoquées (me user:null), login 403, écriture 401, login_blocked tracé, double suspend 409, unsuspend/relogin 200, suspendre admin 409, anti-lockout dernier admin 409, recherche serveur ; régressions — simulateurs 403 (kill-switch), stats 401, home/categories 200.
- ARTÉFACTS DE TEST CORRIGÉS EN COURS : client Prisma périmé dans le serveur dev après db push (redémarrage requis — chunks Turbopack) ; next-server enfant survit au kill du parent next dev (pkill -f "next dev" + "next-server" requis, sinon port 3000 + rate-limiters saturés persistent) ; rate limiters en mémoire (PIN admin 5/15 min, signalements 5/h) saturés par les runs successifs → 429 artefacts.
- DÉPLOIEMENT : commit 41c6eec → GitHub main → update.sh en tant qu'aenews (bun install sans changement, prisma db push en sync, build OK 60 routes, pm2 reload) — PM2 online, home 200, domaine 200.
- VÉRIFICATION PROD (127.0.0.1:3310 + kinshop.aenews.digital) : 45/47 API + 2 artefacts de script re-vérifiés et CONFORMES — F5-2/F5-1/F5-5/F5-6/F5-3/F5-7 intégraux en prod ; F5-4 : chaîne intacte, ≥5 scellées vérifiées, 118 héritées comptabilisées, dernière entrée hash 64 hex (falsification NON rejouée en prod — risque inutile, prouvée en local) ; P1 course 10 coupons parallèles → EXACTEMENT 3×201 + 7×402, 3 en base (les 2 premiers échecs venaient du script : storeId au lieu de slug, percentOff au lieu de value) ; verify facture inconnue = 404 « Facture inconnue » (comportement P4 inchangé) ; simulateurs 403 ; home/categories 200. NAVIGATEUR PROD : accueil rendu 0 erreur ; vitrine boutique de test → badge « Boutique vérifiée par KinShop » visible ; #/admin → onglet Journal → « Vérifier l'intégrité » → verdict « Chaîne intacte — 23 entrée(s) vérifiée(s), 118 héritée(s) » (capture) ; onglet Utilisateurs → boutons Suspendre présents (capture) ; 0 erreur console/page.
- CLEANUP : boutique de test supprimée (cascade), 3 comptes p5prod* + sessions purgés, 0 report résiduel → 0 store/0 user de test (1 user + 1 store réels intacts) ; chaîne d'audit re-vérifiée intacte après cleanup (le journal est immuable par conception — les entrées de test y restent tracées) ; scripts temporaires VPS + locaux supprimés (p5-*.py/sh/js, cookies, /home/z/p5-tmp) ; 0 secret commité.

Stage Summary:
- P5 GOUVERNANCE livré en production (41c6eec) : les quatre piliers sont opérationnels de bout en bout. ① VÉRIFICATION : le propriétaire demande, l'admin tranche, les clients voient le badge « Vérifiée » (vitrine + accueil) — la fonctionnalité morte est devenue un circuit complet. ② COMPTES : l'admin peut suspendre un utilisateur (motif obligatoire, sessions révoquées instantanément, login refusé, fail-closed global) avec gardes anti-lockout et anti-abus. ③ JOURNAL : piste d'audit infalsifiable — chaque entrée scelle la précédente (SHA-256 séquentielle), tout altération/suppression est détectable d'un clic (« Vérifier l'intégrité »), auth incluse (login/échec/logout/register avec IP). ④ SIGNALEMENTS : circuit respectueux du rapporteur (suivi), transitions serveur, décision écrite obligatoire, passerelle directe signalement → suspension.
- Méthode standardisée maintenue : tests API locaux (62/62) → déploiement → tests prod (45/47 + 2 artefacts re-vérifiés conformes) + navigateur → régressions P0/P1/P2/P4 (simulateurs 403, course quotas 3 exact, facture verify, catégories) → purge intégrale (0 résidu) → 0 erreur console.
- Production : kinshop.aenews.digital = GitHub main (41c6eec), PM2 online, 0 résidu de test, 0 erreur.
- Reste P3 (paiements — abstraction provider, dépend de la config Chariow : clé API présente, produit à créer), P6 (boost/home), P7 (analytics uniques) selon le plan marketplace.

---
Task ID: 25
Agent: Z.ai Code (principal)
Task: P6 (plan marketplace) — Boost / Home : expiration paresseuse des campagnes, anti-TOCTOU maxActive, métriques clics/impressions dédupliquées, graphe de transitions admin, sponsorisés honnêtes — implémentation, tests locaux, déploiement, vérification production complète + nettoyage (exigence utilisateur : tests prod systématiques).

Work Log:
- AUDIT PRÉALABLE (6 brèches) : F6-1 HIGH campagnes JAMAIS expirées en DB (active avec endAt passé reste active à vie, pending_payment pourrit le compteur — avec boost.maxActivePerStore=1 le owner était bloqué POUR TOUJOURS dès sa 1re campagne expirée) ; F6-2 HIGH race TOCTOU sur maxActivePerStore (count+create hors tx, famille F-07) ; F6-3 MEDIUM clics/impressions spammables (POST /api/boost/click sans dédup ni rate limit, impressions /api/home comptées à chaque refresh — métriques de ROI falsifiables) ; F6-4 MEDIUM admin boost sans graphe (activate réactivait un rejected, end/reject partout) ; F6-5 LOW accueil : take 6 AVANT filtre catégorie (sponsorisés vides à tort sous ?cat=) + FIFO sans rotation ; F6-6 LOW boutique SANS produit sponsorisable (pub payante vers boutique vide).
- LIB : src/lib/boost.ts — expireDueCampaigns(storeId?) : purge paresseuse (active & endAt<now → ended ; pending_payment > 7 j → expired), best effort, aucune route bloquée.
- API : POST /api/boost → expiration (UX hors tx) + expiration + count + create DANS la transaction quota (withStoreQuotaWrite — mutex par boutique, pattern F-07 ; message 402 cite le vrai compte) ; GET /api/boost → purge avant lecture (statuts réels) ; PATCH owner inchangé (kill-switch F-03) ; boost/click → rate limit 30/5 min/IP + dédup journalier hash boostclk|id|ip|ua|jour via VisitDedup (réponse toujours ok:true, fire-and-forget UX) ; admin/boost → graphe serveur (activate : pending|active|ended, JAMAIS rejected ; reject : pending seul ; end : pending|active) + purge globale GET ; home → purge, take 24 AVANT filtres, sponsorisés exigeant ≥1 produit (store._count.products>0), rotation équitable déterministe par jour (offset dayIndex % n), impressions dédup journalier par campagne×visiteur (hash boostimp|id|ip|ua|jour).
- UI : admin-marketplace-tabs — badges « Expirée », boutons Rejeter (pending) et Relancer (ended) alignés sur le graphe ; BoostPanel — hint honnêteté « La boutique doit avoir au moins 1 produit pour apparaître en Sponsorisé » + labels ended/expired/pending corrects.
- TESTS LOCAUX 56/56 (/home/z/p6p7-tmp) : gardes (401/403 IDOR/400 durée) ; course 8 POST → EXACTEMENT 1×201 + 7×402 ; kill-switch PATCH owner 403 ; graphe admin complet (reject pending 200, activate/reject/end interdits 400, relance ended 200) ; expiration forcée DB (endAt passé) → POST 201 + statuts réels ended/pending (owner débloqué) ; clics dédup 2 visiteurs → 2 clics + rate limit +27 EXACT (budget 30/5 min) ; sponsorisés : sans produit absente / avec 1 produit présente ; impressions : refresh même visiteur inchangé, nouveau visiteur +1 ; P7 intégré (voir Task 26) ; régressions P0 (simulateurs 403), F-06 (clamp), P2 (?cat=).
- DÉPLOIEMENT : commit 9effc84 → GitHub main → update.sh en tant qu'aenews → PM2 online, home 200, domaine 200, visitors30d déjà servi.
- VÉRIFICATION PROD (kinshop.aenews.digital, build 9effc84 puis bd25c88) : 1re passe API depuis le sandbox (cookie Secure → tests via domaine) : F6-2 course 8 → 1×201+7×402 exact ; graphe admin intégral ; F6-1 expiration forcée → POST 201 ; F6-6 sponsorisé sans produit → absent / avec produit → présent ; F7-3 stats uniques exactes ; F7-6 classement B(6)>A ; régressions P0/P4/P5 OK. Découverte méthodologique : l'IP de sortie du sandbox TOURNE (déjà noté en Task 20) → rate limits/dedup ip|ua INTESTABLES depuis le sandbox (clics +40, zéro 429, impressions +1 par requête, uniques gonflés) → re-vérification DEPUIS LE VPS (IP stable) : impressions dedup EXACT (même visiteur stable, nouveau +1) ; clics 2 visiteurs → 2 + rate limit +27 EXACT ; dédup visiteur 3 vues → 1 unique ; rate limit visites 429 présents. P1 course coupons fraîche sur build final → 3×201 + 7×402, EXACTEMENT 3 en DB ; P2 categories clé global = 12 ; P5 chaîne d'audit verdict.intact=true (53 scellées + 118 héritées).
- INCIDENT PROD MAÎTRISÉ (bug réel trouvé par les tests) : cleanup via sqlite3 CLI en 2 invocations séparées (PRAGMA foreign_keys=ON puis DELETE) → pragma mort avec le 1er process → cascade JAMAIS appliquée → campagne active ORPHELINE (storeId sans store) → /api/home 500 « Inconsistent query result: Field store is required » (Prisma include sur relation requise). Réparation : purge orphelins en UNE invocation (pragma+deletes ensemble, heredoc). Durcissement bd25c88 : /api/home RÉSILIENT — filtre relationnel store.status=active DANS le where des campagnes ET des produits (sémantique inner-join SQL, un orphelin ne peut plus faire 500) ; prouvé : orphelin inséré volontairement en local → home 200 (avant : 500). Leçon consignée : TOUT cleanup sqlite3 cascade = 1 seul process.
- NAVIGATEUR PROD (agent-browser) : landing + sections marketplace (Explorer par catégorie 12 catégories, clic 📱 Téléphones → filtre actif, état vide honnête), populaires avec badge « 6 visiteurs » (uniques dédup) ; vitrine AENEWS rendue ; POST /api/analytics/visit 200 visible en console ; badge repassé à « 7 visiteurs » après la visite — chaîne navigateur→analytics→dédup→agrégat→accueil vérifiée de bout en bout ; 0 erreur page/console. NAVIGATEUR LOCAL : onglet Stats → carte « Visiteurs uniques (14 j) » rendue ; onglet Croissance → BoostPanel + hint honnêteté + boutons 7/30 j ; 0 erreur.
- CLEANUP : boutiques/comptes/campagnes/visites/coupons de test purgés (cascade sqlite3 correcte) → 0 store/0 user p6%, 0 orphelin boost/visit/product/coupon ; scripts de test VPS supprimés ; DB locale purgée ; helpers SSH éphémères supprimés ; 0 secret commité.

Stage Summary:
- P6 BOOST/HOME livré en production (9effc84 + durcissement bd25c88) : le cycle de vie des campagnes est désormais complet et honnête — expirations automatiques (fin naturelle / pending abandonné), quota maxActive incompressible même en course, métriques de facturation (impressions, clics) dédupliquées par visiteur×jour et protégées par rate limit, console admin avec graphe de transitions strict (rejet définitif, relance explicite), accueil sans publicité mensongère (boutique vide jamais sponsorisée, rotation équitable entre annonceurs, sections vides honnêtes).
- La vérification a AUSSI prouvé la résilience de l'accueil : même une corruption d'intégrité référentielle (orphelin) ne fait plus tomber la page d'accueil — au lieu de 500, la campagne orpheline est simplement ignorée.
- Production : kinshop.aenews.digital = GitHub main (bd25c88), PM2 online, 0 résidu de test, 0 orphelin, 0 erreur console.

---
Task ID: 26
Agent: Z.ai Code (principal)
Task: P7 (plan marketplace) — Analytics visiteurs uniques : exposition de la métrique G18 (stats + dashboard), anti-spam des visites, exclusion des auto-visites, upsert atomique, rétention de la table de dédup, classement populaires par visiteurs uniques — implémentation, tests, déploiement, vérification production complète + nettoyage (livré en même session que P6, mêmes cycles de test/déploiement).

Work Log:
- AUDIT PRÉALABLE (6 brèches) : F7-1 HIGH /api/analytics/visit SPAMMABLE (aucune rate limit — chaque POST avec (ip|ua) différent créait +1 vue ET +1 visiteur unique → classement populaires et stats falsifiables) ; F7-2 MEDIUM auto-visites du propriétaire comptées (sessionStorage ne limite que par onglet) ; F7-3 HIGH la promesse G18 INVISIBLE : StoreVisit.unique existe depuis V10 mais /api/stats ne l'agrège PAS et l'UI ne l'affiche JAMAIS (le vendeur ne voit jamais ses visiteurs uniques) ; F7-4 LOW VisitDedup croissance infinie (aucune rétention) ; F7-5 LOW upsert StoreVisit non atomique (findUnique→update/create : course sur 1re visite = visite perdue silencieusement) ; F7-6 MEDIUM classement « populaires » par pages vues brutes (falsifiable par refresh) au lieu des visiteurs uniques dédupliqués.
- API : analytics/visit → rate limit 60/5 min/IP (429 explicite), auto-visite du propriétaire → counted:false (session comparée à store.ownerId AVANT tout comptage, n'épuise pas le rate limit), incrément ATOMIQUE (updateMany d'abord — count retourné — create en repli + ré-incrément sur course P2002 : plus aucune visite perdue), purge opportuniste VisitDedup (~2 % des appels, clés > 40 j — le jour étant dans le hash, jamais re-matchées) ; stats → stats.views.uniques {total (clampé planSince F-06), period, série} via un 5e aggregate _sum unique + série uniques depuis les lignes déjà lues (zéro requête série additionnelle) ; home → populaires classés par SOMME DES VISITEURS UNIQUES 30 j (groupBy _sum unique), champ renommé honnêtement visits30d → visitors30d.
- TYPES/UI : VendorStats.views.uniques {total, period, series} ; dashboard — carte « Visiteurs uniques (14 j) » (UserCheck) avec total « sur l'horizon du plan », grille KPI 2/3/5 colonnes responsive ; marketplace-home — badge « N visiteurs » (au lieu de « N visites »), commentaire honnêteté mis à jour.
- TESTS LOCAUX (56/56 avec P6, /home/z/p6p7-tmp) : auto-visite → counted:false ; visite anonyme comptée + re-visite page vue ; même UA autre boutique → compte (clé par slug) ; course 6 premières visites parallèles → toutes counted, uniques.total=6 EXACT (aucune perte, F7-5) ; stats : uniques.total=7 (6 race + 1 visiteur), uniques.period=7, série 7 pts, F-06 plan.statsDays=7 maintenu, uniques.total==views.total sur fenêtre 1 j ; populaires classés par uniques (B 7 avant A) avec champ visitors30d ; rate limit visites → 429 présents (65 requêtes : 47 passées = +47 uniques exactement au budget, 18 refusées — cohérence parfaite relevée a posteriori).
- DÉPLOIEMENT : commits 9effc84 + bd25c88 → update.sh en tant qu'aenews → PM2 online.
- VÉRIFICATION PROD : uniques exactes depuis le VPS (IP stable) : 3 vues même visiteur → views.period=3 ET uniques.period=1 (dédup prouvé) ; auto-visite owner counted:false ; rate limit 60/5 min → 429 présents ; dashboard navigateur : carte « Visiteurs uniques (14 j) » rendue ; E2E navigateur prod : visite de la vitrine AENEWS → POST /api/analytics/visit 200 → badge accueil « 6 visiteurs » → « 7 visiteurs » (incrément réel temps réel) ; régressions P0 (simulateurs 403), P1 (course coupons 3 exact), P2 (12 catégories), P5 (chaîne intacte).
- CLEANUP : intégré à la Task 25 (boutiques/comptes/visites/dedup de test purgés, 0 résidu, 0 orphelin).

Stage Summary:
- P7 ANALYTICS UNIQUES livré en production (bd25c88) : la métrique « visiteurs uniques » (G18) est enfin VISIBLE de bout en bout — collecte dédupliquée (hash slug|ip|ua|jour, aucune donnée personnelle), exposition vendeur (API clampée au plan + carte dashboard), et classement public « populaires » par visiteurs uniques réels au lieu des pages vues falsifiables. Les visites sont protégées contre le spam (rate limit), l'auto-promotion (auto-visites exclues) et la perte silencieuse (upsert atomique) ; la table de dédup a une rétention bornée (40 j).
- État du plan marketplace : P0✅ P1✅ P2✅ P4✅ P5✅ P6✅ P7✅ — RESTE P3 (paiements : abstraction provider, bloqué sur la création du produit Chariow côté console, clé API déjà branchée).
- Production : kinshop.aenews.digital = GitHub main (bd25c88), PM2 online, 0 résidu de test, 0 erreur.
